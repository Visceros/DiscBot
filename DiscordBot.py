...
async def db_auto_clear():
    if datetime.datetime.now().day == 1:
        ...
        # дописать снятие ачивки "накрутчик"
...
async def gkhelp(ctx, arg:str=None):
    ...
    !poll <кол-во опций> - возможность сделать из сообщения опрос. Как использовать: команда пишется,
    используя функцию "ответить" на сообщение из которого хотите сделать опрос (в нём заранее пропишите опции голосования).
    ...
    !warn <пользователь> <кол-во>- выдать кому-то предупреждение и снять 3 минуты активности. Аргументом <пользователь> может 
    являться айди юзера, его имя, или упоминание. Можно выдать несколько предупреждений поставив после пользователя пробел и число.\n
    !poll <кол-во опций> <длительность> - возможность сделать из сообщения опрос. Как использовать: команда пишется,
    используя функцию "ответить" на сообщение из которого хотите сделать опрос (в нём заранее пропишите опции голосования). 
    Сообщение, на которое вы отвечаете, дублируется и к нему в виде реакций назначаются опции для голосования (до 9 опций). 
    Базовая длительность - час.\n
    ...
    !bingo <число> - бот составит число из указанного количества цифр, например !bingo 4 - даст число из 5 цифр. Исходно - 3 цифры.\n
    !buy <название> - купить товар\n
    !shop add <название> <цена> <длительность> - добавить товар (только администраторы) - укажите название товара,  
    его цену и длительность, если это рамка для профиля или роль\n
    !shop delete <название> или <ID>- удалить товар из магазина (только администраторы)\n```
        """
    if arg==None:
        await ctx.send(basic_help)
    elif arg=="mod":
